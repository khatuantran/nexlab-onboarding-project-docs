...
- **(US-009 delete amend, 2026-05-16)** When an authenticated user calls `DELETE /api/v1/me/avatar`, the system shall best-effort destroy the existing Cloudinary asset (parse publicId from `users.avatar_url`; log warn on Cloudinary failure but proceed), clear `users.avatar_url` to `null`, and return 204 No Content. Idempotent — DELETE on an already-null avatar still returns 204 without invoking Cloudinary.
...
- **(Delete amend, 2026-05-16)** When an authenticated user calls `DELETE /api/v1/me/cover`, the system shall best-effort destroy the Cloudinary asset (parse publicId from `users.cover_url`; log warn on failure but proceed), clear `users.cover_url` to `null`, and return 204. Idempotent — DELETE on null cover returns 204 without invoking Cloudinary.
- **(Delete amend, 2026-05-16)** When an admin calls `DELETE /api/v1/projects/:slug/cover`, the system shall require `requireAuth + requireAdmin`, resolve slug (404 `PROJECT_NOT_FOUND` if missing), best-effort destroy the Cloudinary asset, clear `projects.cover_url`, and return 204. Non-admin → 403 `FORBIDDEN`. Idempotent on null.
