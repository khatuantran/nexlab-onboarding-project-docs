...
> **v4.7 (US-011, 2026-05-16)** — `ActivityRail` drops `STATIC_PADDING` dummy entries ("Ngọc Linh" / "Phương Trâm"). Renders only real `sections.updated_*` projection (max 4). New empty state "Chưa có hoạt động — feature này chưa được chỉnh sửa." when no section has been edited.
>
