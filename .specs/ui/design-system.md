...
| 2026-05-16 | **CR-006 (v4.6) — `ConfirmDialog` primitive** registered (`apps/web/src/components/common/ConfirmDialog.tsx`). Radix Dialog wrapper: title + description + Huỷ/Xác nhận buttons + `variant: "destructive" \| "default"` + `pending`. Destructive variant prepends `AlertTriangle` plate (size-10 rounded-[10px] bg-destructive/12). Replaces `window.confirm()` for archive actions on `ProjectActionsMenu` (US-004) + `FeatureActionsMenu` (US-008). Other confirm() call sites (dirty-form discard / user disable / AdminSettings danger zone) NOT migrated yet.                                                                                                                                                                                           | ConfirmDialog v4.6 `97e4fc4`                 |
| 2026-05-16 | **Cover image hero treatment** ([US-019](../stories/US-019.md)). Add inline `CoverUploadDialog` (ProfilePage) + `ProjectCoverUploadDialog` (ProjectLandingPage admin actions) — Radix Dialog wrapper around `<input type=file accept=image/png,image/jpeg,image/webp>` with 4 MB client-side cap. ProfilePage hero swaps `<GradientHero>` for `<img class=object-cover>` + `bg-gradient-to-b from-black/40 to-black/60` overlay khi `coverUrl` set; ProjectHero giữ `<GradientHero>` (blobs glow) + layer image+overlay absolute bên trong. Replaces 2 `placeholderToast` surfaces. Implementation inline (apps/web/src/pages/ProfilePage.tsx + ProjectLandingPage.tsx) — promote to `components/profile/CoverUploadDialog.tsx` shared khi có ≥ 3 call site. | US-019 `ca6cea8`                             |
| 2026-05-16 | **Hero chip + secondary text contrast pass** ([BUG-005](../bugs/BUG-005.md) + [BUG-006](../bugs/BUG-006.md)). Standardize 2 patterns on dark hero gradient: (a) secondary text (meta line / HeroStat label / login byline) → `text-white/75`; (b) chip text → Tailwind `*-100` shade (lightness ~95%) cho mọi tone (orange / green / blue / rose / amber) + `bg-*-500/35` + `border-*-500/55`. Replaces drift `text-white/45..55` cho meta và `text-[#FFD092]` / `text-*-200` + `bg-*/[0.18..0.25]` cho chip. Áp dụng site-wide: HomePage, ProjectHero, FeatureDetailPage, SearchPage, AdminUsersPage, AdminSettingsPage, LoginBrandPanel.                                                                                                                   | BUG-005 `a77d254` + BUG-006 v2 `941bc36`     |
