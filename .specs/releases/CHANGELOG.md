...
- **Cover image upload — profile + project** ([US-019](../stories/US-019.md), 2026-05-16) — Promoted from [BL-006](../backlog/BL-006.md). New `FR-COVER-001`. Migration `0013_users_projects_cover_url.sql` adds nullable `users.cover_url` + `projects.cover_url`. 2 new endpoints (mirror US-009 avatar pattern): `POST /api/v1/me/cover` (`requireAuth`, max 4 MB png/jpg/webp, Cloudinary folder `covers/users/`) + `POST /api/v1/projects/:slug/cover` (admin gate, folder `covers/projects/`). Read endpoints (`GET /me`, `GET /projects`, `GET /projects/:slug`) augment response với `coverUrl: string | null`. FE: new `CoverUploadDialog` (clone AvatarUploadDialog shape) thay `placeholderToast("Đổi ảnh bìa")` trên ProfilePage; ProjectHero `<img>` + overlay `bg-gradient-to-b from-black/40 to-black/60` khi `coverUrl` set, blobs glow giữ nguyên. Reuses Cloudinary error contracts (FILE_TOO_LARGE / UNSUPPORTED_MEDIA_TYPE / UPLOAD_PROVIDER_ERROR / UPLOADS_DISABLED) — no new error codes.

- **Profile skills CRUD** ([US-018](../stories/US-018.md), 2026-05-16) — Promoted from [BL-002](../backlog/BL-002.md). New `user_skills` table (migration `0012`) with `(user_id, lower(label))` unique index for case-insensitive dedupe. Shared `SKILL_COLORS` 7-tuple enum + `updateSkillsRequestSchema` with `.max(12)` + dedupe refine. New endpoints `GET /api/v1/me/skills` (ordered list) + `PUT /api/v1/me/skills` (replace-all semantics in single transaction). FE: `SkillsCard` reads `useMySkills()` real chips with 7-hue palette; new `EditSkillsDialog` with rows (label input + color dropdown + up/down reorder + X remove + "Thêm skill" capped at 12). Hardcoded 7-chip array retired. New FR-PROFILE-002. **12 of 13** mock surfaces cleared (Sprint 4a). Commits: `37edd6b` (T1 migration + shared) → `e799165` (T2 BE) → `f846045` (T3 FE) → progress sync.

- **Workspace stats hero + `filledSectionCount` per project** ([US-014](../stories/US-014.md), 2026-05-16) — New `GET /api/v1/workspace/stats` returns `{ projectCount, featuresDocumented, contributorsActive }` with a 30-day active window. `ProjectSummary` gains `filledSectionCount`. FE HomePage hero is now **3 tiles** (Projects / Đủ tài liệu / Đóng góp 30 ngày) reading the endpoint; the placeholder "Onboard TB=2.3h" tile is gone per 2026-05-16 triage. "Đủ doc" filter chip uses `filledSectionCount === featureCount * 5` instead of `featureCount > 0` proxy. New FR-STATS-001. 4 mock surfaces cleared this story / sprint. Commits: `71c2570` (BE) → `cd1b066` (FE) → progress sync.

- **Profile real stats + recent projects + activity feed** ([US-015](../stories/US-015.md) + [US-016](../stories/US-016.md) + [US-017](../stories/US-017.md), 2026-05-16) — Three new endpoints under `/me`: `GET /me/stats` (4 per-user counts), `GET /me/recent-projects?limit=` (user's touched projects ranked by latest edit), `GET /me/activity?limit=&cursor=` (cursor-paginated section-edit feed, verb fixed `'updated'` v1). `userStatsRepo` aggregates all three via `sections.updated_by`-keyed queries, excluding archived features + projects. FE `ProfilePage` `StatsCard` / `RecentProjectsCard` / `ActivityFeedCard` swap hardcoded `4/18/42/12` + 4-fake-rows + 4-fake-activity for real hook data with skeleton-on-load + empty states. `ActivityRail` "Xem tất cả" → right-side drawer (Radix Dialog repositioned) showing all 5 sections sorted by recency, replacing `placeholderToast("Lịch sử hoạt động đầy đủ...")`. New FR-PROFILE-001 + FR-ACTIVITY-001. Sprint 3 Phase 4 — clears **4 of 13** mock surfaces (Profile stats + Profile recent + Profile activity + ActivityRail Xem tất cả). Cumulative: **11 of 13** cleared. Commits: `920a601` (BE for all 3) → `f66f79a` (FE for all 3) → progress sync.

- **Edit feature metadata (title + slug)** ([US-012](../stories/US-012.md), 2026-05-16) — Fills FR-FEAT-001's "edit title or slug" gap. New `PATCH /api/v1/projects/:slug/features/:fSlug` admin-only route accepting `{ title?, slug? }` (≥ 1 key, Zod refine). Slug rename invalidates the old URL (404 on legacy bookmarks — no redirect v1); slug collision within the same project surfaces as 409 `FEATURE_SLUG_TAKEN`. FE: `FeatureActionsMenu` grows a "Sửa feature" item; new `EditFeatureDialog` mirrors `EditProjectDialog` shape; `useUpdateFeature` mutation invalidates project + feature queries. 6 shared + 6 repo + 9 route integration tests + 3 dialog tests. Sprint 2 Phase 3a. Commits: `a6d4798` (T1) → `782e008` (T2) → `7eb3c3a` (T3) → this commit (sync).

- **External repo URL + PR URL linking** ([US-013](../stories/US-013.md), 2026-05-16) — Promoted from [BL-001](../backlog/BL-001.md). New FR-LINK-001. Migration `0011_projects_repo_url_features_pr_url.sql` adds nullable `projects.repo_url` + `features.pr_url`. Shared `urlSchema` accepts any `http(s)` URL up to 500 chars (loose per 2026-05-16 triage). `PATCH /projects/:slug` and `PATCH /features/:fSlug` accept `repoUrl?` / `prUrl?` with undefined-skip / null-clear / string-update semantics. Read endpoints expose new fields. FE: `ProjectLandingPage` "Repo" button + `FeatureDetailPage` "PR" button become real `<a target="_blank" rel="noopener noreferrer">` when URL set, disabled `<button>` with tooltip when null — `placeholderToast("Mở repo"/"Xem PR")` retired. `EditProjectDialog` + `EditFeatureDialog` gain URL inputs; empty input clears (sends null). 5 shared + 10 route integration + 4 FE page tests. Sprint 2 Phase 3b — clears 2 of 13 mock surfaces. Commits: `944257f` (T1) → `7dfb32c` (T2) → `7f5a9ec` (T3) → this commit (sync).

- **Profile enrichment — phone / department / location / bio** ([US-010](../stories/US-010.md), 2026-05-16) — `PATCH /api/v1/me` accepts 4 optional nullable fields (`phone` Vietnamese-style regex, `department` ≤ 120, `location` ≤ 120, `bio` ≤ 500). Migration `0010_users_profile_enrichment.sql` adds the columns. `GET /me` augments AuthUser shape with the 4 fields. FE ProfilePage `PersonalInfoCard` now renders real values (or italic "— Chưa cập nhật" when null) instead of hardcoded "0901 234 567" / "Product · BA Team" / "TP. Hồ Chí Minh". `EditProfileDialog` extended from 1 input → 5 inputs (textarea for bio). Phase 1a of Real-data backfill (CR-006 v4.6 mock audit). FR-USER-003 amend. 209/209 API + 161/161 web + 24/24 shared. Commits: `a8b559c` (BE) → `07a32a2` (FE) → this commit (sync).

- **Real contributors derived from `sections.updated_by`** ([US-011](../stories/US-011.md), 2026-05-16) — New FR-PROJ-003. `GET /projects`, `GET /projects/:slug`, `GET /projects/:slug/features/:fSlug` augment response with `contributors: ContributorSummary[]` (max 5, sorted by recency, archived features excluded for project scope). Repo: `getContributorsForProject` / `getContributorsForFeature` / `getContributorsForProjects` (batched window query, avoids N+1). FE drops hardcoded contributor data from 4 surfaces: `ProjectCard` `CONTRIBUTOR_POOL` + `pickContributors` gone; `FeatureCard` `["TM","NL"]` gone; `ProjectHero` `["TM","NL","PT","HD"]` gone; `ActivityRail` `STATIC_PADDING` (Ngọc Linh / Phương Trâm) gone. Zero schema change. Phase 1b of Real-data backfill. 214/214 API + 161/161 web. Commits: `5816bf8` (BE) → `d436207` (FE) → this commit (sync).

- **Self-service profile** ([US-009](../stories/US-009.md), 2026-05-15) — Mọi authenticated user nay có page `/profile` riêng để (a) xem email + displayName + role + lastLoginAt + createdAt, (b) edit displayName inline, (c) đổi mật khẩu (3-field, validate mismatch + len ≥ 8 client-side, server bcrypt.compare oldPassword), (d) upload avatar PNG/JPG/WebP ≤ 2 MB qua Cloudinary CDN. UserMenu "Hồ sơ của tôi" item enabled (drop "Sắp ra mắt" placeholder) → `Link to="/profile"`. Migration `0009_users_avatar_url.sql` thêm nullable `users.avatar_url`. BE mount `/api/v1/me` với 4 endpoint (`GET`, `PATCH`, `POST /password`, `POST /avatar`) — luôn đọc `req.session.userId`, không nhận `:id` URL param → IDOR-proof by construction. Password change purges other Redis sessions while preserving the current sid (`purgeSessionsForUserExcept(redis, userId, exceptSid)`). `Avatar` component gains `imageUrl` prop + `lg` (80px) size; UserMenu trigger + AppHeader pass `user.avatarUrl` through. AuthUser shape extended với 3 field (avatarUrl, lastLoginAt, createdAt) — backwards compatible via wildcard exports. FR-USER-003 mới. 19 new api tests + 12 new web tests + new `e2e/us-009.spec.ts`. **203/203 API + 24 shared + 154 web** unit tests xanh. Commits: `f846a57` (spec) → `477a410` (BE) → `ef3b59a` (FE) → this commit (E2E + sync).
- **Admin feature archive** ([US-008](../stories/US-008.md), 2026-05-15) — Admin nay có thể archive 1 feature trong project qua overflow ⋯ menu trên `FeatureCard`. Soft-delete: migration `0008_features_archived_at.sql` thêm `archived_at` nullable + partial index `features_active_idx`. `featureRepo.archive` (idempotent) + filter `archived_at IS NULL` ở `findByProjectAndSlug` + `projectRepo.listFeatures`. Route `POST /api/v1/projects/:slug/features/:fSlug/archive` (admin-only, 204/403/404). FE: `FeatureActionsMenu` single-item dropdown + `useArchiveFeature` mutation + admin overlay trên `FeatureCard` (hide chevron per BUG-004 contract). FR-FEAT-001 amend (+3 EARS). 184 API + 24 shared + 142 web + new `e2e/us-008.spec.ts` xanh. Commits: `2f83cd9` (spec) → `02edde1` (BE) → `a03c345` (FE) → this commit (E2E + sync).
...
- **Archive confirm now uses styled `ConfirmDialog`** (CR-006 v4.6, 2026-05-16) — Admin actions "Lưu trữ project" (US-004) và "Lưu trữ feature" (US-008) trước đây dùng native `window.confirm()` (giao diện nâu xám của browser, không follow design system). Thay bằng `ConfirmDialog` (Radix Dialog + AlertTriangle plate + Huỷ/Lưu trữ buttons + pending state). Behavior + endpoints + cache-invalidate flow giữ nguyên. Commit `97e4fc4`.
