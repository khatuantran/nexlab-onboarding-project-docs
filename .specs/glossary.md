...
- **Upload** — File ảnh (png/jpg/webp ≤ 5 MiB) gửi qua `POST /api/v1/features/:id/uploads`, stream lên **Cloudinary CDN** (per [CR-004](changes/CR-004.md) Phase 2). Response trả absolute `https://res.cloudinary.com/...` URL; FE render trực tiếp qua `<img src>`, không qua BE proxy. DB row giữ `cloudinary_public_id` cho audit. Xem [FR-UPLOAD-001](02-requirements.md#fr-upload-001--image-upload-for-screenshots).
